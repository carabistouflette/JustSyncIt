...
public class WatchServiceThreadPool extends ManagedThreadPool {
    ...
    protected BlockingQueue<Runnable> createWorkQueue() {
        // Watch service benefits from smaller queue to prevent event buildup
        return new LinkedBlockingQueue<>(Math.max(50, poolConfig.getQueueCapacity()));
    }
    
    @Override
    protected ThreadFactory createThreadFactory() {
        return new WatchServiceThreadFactory(poolConfig.getThreadNamePrefix(), 
                                         poolConfig.getPriority().getValue());
    }
    
    @Override
    protected RejectedExecutionHandler createRejectionHandler() {
        // For watch service, prefer caller runs to ensure events are processed
    ...
    public void triggerAdaptiveResizing() {
        ...
        if ((eventRate > 1.0 || watcherLoad > 2.0) && maxSize < poolConfig.getMaximumPoolSize()) {
            ...
            logger.debug("Adaptive resize: increased WatchService pool max size to {} (eventRate: {:.2f}, watcherLoad: {:.2f})",
            ...
            logger.debug("Adaptive resize: decreased WatchService pool max size to {} (eventRate: {:.2f}, watcherLoad: {:.2f})",
                       newMaxSize, eventRate, watcherLoad);
        }
    }
