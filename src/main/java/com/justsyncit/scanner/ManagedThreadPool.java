 ...
 * Provides common functionality for all specialized thread pool implementations.
 */
public abstract class ManagedThreadPool {
    ...
    /**
     * Creates the appropriate work queue for this pool type.
     */
    protected BlockingQueue<Runnable> createWorkQueue() {
        // Use different queue types based on pool configuration
        if (poolConfig.getQueueCapacity() == 0) {
            return new SynchronousQueue<>();
        } else if (poolConfig.getQueueCapacity() < 0) {
            // Unbounded queue
            return new LinkedBlockingQueue<>();
        } else {
            // Bounded queue
            return new LinkedBlockingQueue<>(poolConfig.getQueueCapacity());
        }
    }
    
    /**
     * Creates a thread factory for this pool.
     */
    protected ThreadFactory createThreadFactory() {
        return new ManagedThreadFactory(poolType.getName(), poolConfig.getPriority().getValue());
    }
    
    /**
     * Creates a rejection handler for this pool.
     */
    protected RejectedExecutionHandler createRejectionHandler() {
        return new ThreadPoolExecutor.CallerRunsPolicy(); // Default: run in caller thread
    }
    ...
    private static class ManagedThreadFactory implements ThreadFactory {
        ...
        ManagedThreadFactory(String poolName, int priority) {
        ...
        public Thread newThread(Runnable r) {
            ...
            t.setUncaughtExceptionHandler((thread, ex) -> {
                LoggerFactory.getLogger(ManagedThreadFactory.class)
                        .error("Uncaught exception in thread {}", thread.getName(), ex);
            });
