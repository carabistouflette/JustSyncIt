...
public class BatchProcessingThreadPool extends ManagedThreadPool {
    ...
    protected BlockingQueue<Runnable> createWorkQueue() {
        // Batch processing benefits from moderate queue capacity
        return new LinkedBlockingQueue<>(Math.max(200, poolConfig.getQueueCapacity()));
    }
    
    @Override
    protected ThreadFactory createThreadFactory() {
        return new BatchProcessingThreadFactory(poolConfig.getThreadNamePrefix(), 
                                            poolConfig.getPriority().getValue());
    }
    
    @Override
    protected RejectedExecutionHandler createRejectionHandler() {
        // For batch processing, prefer caller runs to prevent batch loss
    ...
    public void triggerAdaptiveResizing() {
        ...
        if (loadFactor > 0.75 && avgBatchSize > 5.0 && maxSize < poolConfig.getMaximumPoolSize()) {
            ...
            logger.debug("Adaptive resize: increased BatchProcessing pool max size to {} (load: {:.2f}, avgBatch: {:.1f})",
            ...
            logger.debug("Adaptive resize: decreased BatchProcessing pool max size to {} (load: {:.2f}, avgBatch: {:.1f})",
                       newMaxSize, loadFactor, avgBatchSize);
        }
    }
