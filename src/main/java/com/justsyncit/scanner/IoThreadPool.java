 ...
 * Optimized for AsynchronousFileChannel operations with appropriate queue
 * sizing
...
public class IoThreadPool extends ManagedThreadPool {
    ...
    public IoThreadPool(ThreadPoolConfiguration.PoolConfig poolConfig,
        ...
        super(poolConfig, systemInfo, monitor, ThreadPoolManager.PoolType.IO,
                new LinkedBlockingQueue<>(Math.max(500, poolConfig.getQueueCapacity())),
                new IoThreadFactory(poolConfig.getThreadNamePrefix(), poolConfig.getPriority().getValue()),
