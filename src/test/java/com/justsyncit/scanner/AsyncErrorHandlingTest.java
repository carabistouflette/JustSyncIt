...
class AsyncErrorHandlingTest extends AsyncTestBase {
    ...
    void shouldHandlePoolExhaustionGracefully() {
        AsyncTestUtils.runWithIsolatedPool(2, (pool) -> {
            try {
                // Acquire all buffers
                ByteBuffer buffer1 = AsyncTestUtils.getFutureResult(pool.acquireAsync(8192), SHORT_TIMEOUT.dividedBy(2));
                ByteBuffer buffer2 = AsyncTestUtils.getFutureResult(pool.acquireAsync(8192), SHORT_TIMEOUT.dividedBy(2));
                
                // Try to acquire one more - this should timeout or fail
                CompletableFuture<ByteBuffer> excessFuture = pool.acquireAsync(8192);
                try {
                    ByteBuffer buffer = AsyncTestUtils.getFutureResult(excessFuture, SHORT_TIMEOUT.dividedBy(50)); // Very short timeout
    ...
    void shouldHandleNonExistentFileChunking() {
        AsyncTestUtils.runWithIsolatedPool(3, (pool) -> {
            try {
                // Create file chunker
                com.justsyncit.hash.Blake3Service blake3Service = new com.justsyncit.ServiceFactory().createBlake3Service();
                ...
                try {
                    ...
                    CompletableFuture<FileChunker.ChunkingResult> future = chunker.chunkFileAsync(nonExistentFile, new FileChunker.ChunkingOptions());
                    
                    FileChunker.ChunkingResult result = AsyncTestUtils.getFutureResult(future, SHORT_TIMEOUT);
                    // The result should indicate failure
                    assertFalse(result.isSuccess());
                    assertNotNull(result.getError());
                    // The error should be an IllegalArgumentException with message about file not existing
        ...
        AsyncTestUtils.runWithIsolatedPool(3, (pool) -> {
            try {
                ...
                try {
                    ...
                    CompletableFuture<FileChunker.ChunkingResult> future = chunker.chunkFileAsync(null, new FileChunker.ChunkingOptions());
    ...
    void shouldHandleChunkProcessingErrors() {
        AsyncTestUtils.runWithIsolatedPool(3, (pool) -> {
            try {
                ...
                CompletableFuture<String> future = faultyHandler.processChunkAsync(testBuffer, 0, 1, Path.of("test.txt"));
