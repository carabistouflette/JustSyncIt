 ...
 * Supports both synchronous and asynchronous file generation for testing scenarios.
 */
public final class AsyncTestFileGenerator {
    ...
    public static class FileGenerationConfig {
        ...
        public boolean isUseAsyncIO() { return useAsyncIO; }
        public String getRepeatingPattern() { return repeatingPattern; }
     ...
     */
    public static CompletableFuture<FileGenerationResult> generateFileAsync(Path filePath, FileGenerationConfig config) {
     ...
     */
    private static void fillBufferRange(ByteBuffer buffer, int start, int end, java.util.function.IntSupplier valueSupplier) {
        int originalPosition = buffer.position();
        buffer.position(start);
        int limit = Math.min(end, buffer.capacity());
        buffer.limit(limit);
     ...
     */
    public static CompletableFuture<FileGenerationResult[]> generateFilesAsync(Path baseDir, FileGenerationConfig[] configs) {
        return CompletableFuture.supplyAsync(() -> {
            try {
            ...
            CompletableFuture<FileGenerationResult>[] futures = new CompletableFuture[configs.length];
            
            for (int i = 0; i < configs.length; i++) {
                String fileName = "test_file_" + i + "_" + configs[i].getPattern().name().toLowerCase() + ".dat";
                Path filePath = baseDir.resolve(fileName);
                futures[i] = generateFileAsync(filePath, configs[i]);
            }
            
            return CompletableFuture.allOf(futures)
                .thenApply(v -> {
                    FileGenerationResult[] results = new FileGenerationResult[futures.length];
    ...
    public static FileGenerationConfig[] getPerformanceConfigs() {
        return new FileGenerationConfig[] {
            ...
            new FileGenerationConfig(4194304, ContentPattern.SEQUENTIAL, 131072, true, "PERF", "Performance test data."),
            new FileGenerationConfig(16777216, ContentPattern.MIXED, 262144, true, "PERF", "Performance test data."),
            new FileGenerationConfig(67108864, ContentPattern.RANDOM, 524288, true, "PERF", "Performance test data.")
        };
    }
}